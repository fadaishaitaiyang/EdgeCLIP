...
def fit_one_epoch(model_train, model, yolo_loss, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen,
    ...
    if train_mode==0:
        torch.save(model.state_dict(),
                   './data/logsSynth_1/ep%03d-loss%.3f-val_loss%.3f.pth' % (epoch + 1, loss / epoch_step, val_loss / epoch_step_val))
    if train_mode == 1:
        torch.save(model.state_dict(),
                   './data/logsReal_1/ep%03d-loss%.3f-val_loss%.3f.pth' % (
...
def compute_heatmap_loss_by_gtbox_predheatmap(box, heatmap):
    ...
    for i in range(batch_size):
        ...
        # eng_loss = F.cross_entropy(power,total_power)
