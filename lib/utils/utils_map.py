...
def get_map(MINOVERLAP, DATASET,getwUOC,draw_plot, path='./map_out'):
    if DATASET==1:
        ...
        IMG_PATH = os.path.join(path, 'images-real-optional')
        TEMP_FILES_PATH = os.path.join(path, '.temp_files_real')
        RESULTS_FILES_PATH = os.path.join(path, 'real-results')
    if DATASET==0:
        GT_PATH = os.path.join(path, 'ground-truth-synth')
        DR_PATH = os.path.join(path, 'detection-synth-results')
        IMG_PATH = os.path.join(path, 'images-synth-optional')
        TEMP_FILES_PATH = os.path.join(path, '.temp_files_synth')
        RESULTS_FILES_PATH = os.path.join(path, 'synth-results')
...
def preprocess_gt(gt_path, class_names):
    ...
    for i, image_id in enumerate(image_ids):
        ...
        for line in lines_list:
            ...
            bbox = [left, top, right - left, bottom - top, difficult, int(image_id), cls_id,
...
def get_coco_map(DATASET,class_names, path):
    from pycocotools.coco import COCO
    from pycocotools.cocoeval import COCOeval
    if DATASET==1:
        GT_PATH = os.path.join(path, 'ground-truth-real')
        DR_PATH = os.path.join(path, 'detection-real-results')
        COCO_PATH = os.path.join(path, 'coco_eval_real')
    if DATASET==0:
        GT_PATH = os.path.join(path, 'ground-truth-synth')
        DR_PATH = os.path.join(path, 'detection-synth-results')
        COCO_PATH = os.path.join(path, 'coco_eval_synth')
